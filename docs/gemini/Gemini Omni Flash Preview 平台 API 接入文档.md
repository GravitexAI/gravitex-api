...
| 能力 | 官方支持 | 本平台当前 | 说明 |
| --- | --- | --- | --- |
| 文生视频 | 支持 | 支持 | `input` 传文本，内部使用 `text_to_video` |
| 图生视频 | 支持 | 支持单图 | `input` 传文本和一张 Base64 图片，内部使用 `image_to_video` |
| 视频输出 | 支持 | 支持 | 返回最终视频地址 |
| 视频音频 | 支持 | 支持 | 视频输出包含音频，平台记录 `generate_audio` |
| 异步任务 | 支持 | 支持 | 提交任务后通过 GET 轮询 |
| 同步阻塞调用 | 支持 | 不支持 | 官方 Interactions API 可不设置 `background`；平台统一走后台任务 |
| SSE 流式输出 | 支持 | 不支持 | 官方 Interactions API 支持 `stream`；平台当前未映射流式事件 |
| 视频编辑 | 支持 | 不支持 | 官方使用 `video_config.task=edit`；平台当前未接入 |
| 多图参考 | 支持 | 不支持 | 官方支持参考素材/多图；平台当前只处理单张图片 |
