...
func TestEffectiveBillingUsagePreservesGeminiVideoModality(t *testing.T) {
	usage := &dto.Usage{
		BillingUsage: dto.NewGeminiChatBillingUsage(&dto.GeminiUsageMetadata{
			...
			PromptTokensDetails: []dto.GeminiPromptTokensDetails{
				...
				{Modality: "VIDEO", TokenCount: 1100},
			},
	...
	assert.Equal(t, 1200, got.PromptTokensDetails.VideoTokens)
	assert.Equal(t, 100, got.PromptTokensDetails.ImageTokens)
	assert.Equal(t, 200, got.PromptTokensDetails.TextTokens)
	assert.Zero(t, usage.PromptTokensDetails.VideoTokens, "canonicalization must not mutate the response usage")
}
