...
func IsAutoModel(name string) bool {
	base := stripAutoCompactSuffix(name)
	return base == autoVirtualModel || strings.HasPrefix(base, autoVirtualModel+":")
}
...
func ParseAutoModelName(name string) (autoName string, compact bool) {
	compact = strings.HasSuffix(name, ratio_setting.CompactModelSuffix)
	return stripAutoCompactSuffix(name), compact
}
...
func stripAutoCompactSuffix(name string) string {
	return strings.TrimSuffix(name, ratio_setting.CompactModelSuffix)
}
