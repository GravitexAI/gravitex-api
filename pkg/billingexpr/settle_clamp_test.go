...
func TestComputeTieredQuotaRetainsInt32LimitForTokensAndTasks(t *testing.T) {
	for _, taskUsage := range []bool{false, true} {
		name := "tokens"
		expression := `tier("base", 4294967296)`
		if taskUsage {
			name = "task"
			expression = `tier("base", 4294.967296)`
		}
