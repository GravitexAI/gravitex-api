...
### 4.0 手动上传后执行这一条（推荐记住这个）

自己用 zip / SFTP / 任何方式把新的 `claude-platform-test` 传到服务器之后，
**在服务器上执行**：
...
就这一条，不用记别的。它是幂等的，随便跑多少次都安全，会自动：

1. 清掉 macOS 夹带的垃圾（`._*` / `__MACOSX` / `.DS_Store` / `__pycache__`）
2. **检查两个 venv** —— 坏了或没有才重建，好的不动（省时间）
3. 装/更新依赖（所以改了 `requirements.txt` 也不用额外操作）
4. 跑配置校验（不发网络请求、不花钱），失败就中止不重启
5. 重启服务并验收，最后打印 `✅ 全部就绪` 或 `❌` + 原因

**三种情况都已实测验证过**：

| 情况 | 脚本行为 |
|---|---|
| venv 正常 | `venv 正常（Python 3.9.25），不重建` —— 跳过，几秒完成 |
| venv 是 Mac 传来的断链 | 打印当前指向 `/Library/Developer/...`，自动重建 |
| venv 整个不存在（SFTP 替换文件夹会这样） | `没有 venv，新建中`，自动建好 |

> **传之前可以不用管 `.venv`** —— 带上来了脚本会自动重建，没带上来也会自动新建。
> 当然不传更快（能省 20 多 MB 和上千个文件）。
