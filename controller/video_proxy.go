...
func VideoProxy(c *gin.Context) {
	...
	if descriptor == nil && !taskHasPluginExecution(task) {
		...
		case constant.ChannelTypeGemini:
			...
			contentURL, err = getGeminiVideoURL(channel, task, key)
		...
		if contentURL != "" {
			descriptor = &relaychannel.TaskContentRequest{URL: contentURL, Method: c.Request.Method, Headers: headers, Credentialless: len(headers) == 0}
		}
	}
	if descriptor == nil {
