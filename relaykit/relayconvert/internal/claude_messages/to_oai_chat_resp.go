...
func (s *ClaudeToChatStreamState) ConvertChunk(claudeResponse *dto.ClaudeResponse) (*dto.ChatCompletionsStreamResponse, error) {
	...
	if s.toolArgsSeen == nil { s.toolArgsSeen = make(map[int]bool) }
	...
	case "content_block_delta":
		...
		converted.Index = kitutil.GetPointer(toolIndex)
		if *claudeResponse.Delta.PartialJson != "" { s.toolArgsSeen[contentBlockIndex] = true }
	case "content_block_stop":
		if claudeResponse.Index != nil {
			idx := *claudeResponse.Index
			if toolIndex, ok := s.toolIndexByContentBlock[idx]; ok && s.blockTypeByContentBlock[idx] == "tool_use" && !s.toolArgsSeen[idx] {
				delete(s.blockTypeByContentBlock, idx)
				delete(s.toolArgsSeen, idx)
				return StreamResponseClaude2OpenAI(&dto.ClaudeResponse{
					Type: "content_block_delta", Index: kitutil.GetPointer(toolIndex),
					Delta: &dto.ClaudeDelta{Type: "input_json_delta", PartialJson: kitutil.GetPointer("{}")},
				}), nil
			}
