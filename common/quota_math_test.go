...
func TestSingleRequestQuotaRejectsValuesAllowedForWallets(t *testing.T) {
	value := int64(math.MaxInt32) + 1
	quota, err := WalletQuotaFromDecimalStrict(decimal.NewFromInt(value))
	require.NoError(t, err)
	assert.Equal(t, int(value), quota)

	for _, convert := range []struct {
		name string
		run  func() (int, error)
	}{
		{"float", func() (int, error) { return QuotaFromFloatStrict(float64(value)) }},
		{"round", func() (int, error) { return QuotaRoundStrict(float64(value)) }},
		{"decimal", func() (int, error) { return QuotaFromDecimalStrict(decimal.NewFromInt(value)) }},
	} {
		t.Run(convert.name, func(t *testing.T) {
			quota, err := convert.run()
			var clamp *QuotaClamp
			require.ErrorAs(t, err, &clamp)
			assert.Zero(t, quota)
			assert.Equal(t, math.MaxInt32, clamp.Clamped)
			assert.Equal(t, QuotaClampOverflow, clamp.Kind)
		})
	}
}
