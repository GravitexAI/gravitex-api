...
// GetChannelsByGroupAndType returns enabled channels of the given type accessible
// by the group, independent of model names and abilities. Cache and DB paths
// both match complete group names and use priority DESC, id ASC ordering.
func GetChannelsByGroupAndType(group string, channelType int) ([]*Channel, error) {
	var channels []*Channel
	if common.MemoryCacheEnabled {
		channelSyncLock.RLock()
		defer channelSyncLock.RUnlock()
		for _, channel := range channelsIDM {
			if channel.Type == channelType && channel.Status == common.ChannelStatusEnabled {
