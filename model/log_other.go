...
// NewLogOtherFromMap imports internal legacy billing metadata while preserving
// its stored shape. Never use this with untrusted client-supplied metadata.
// Role projections still hide the fork's historical cost and stream fields.
func NewLogOtherFromMap(values map[string]any) *LogOther {
	o := NewLogOther()
