...
var passthroughSkipHeaderNamesLower = map[string]struct{}{
	...
	// anthropic-beta / anthropic-version 有渠道特定语义：Bedrock 要把它们搬到
	// body 字段，Vertex 要按白名单过滤，Claude 直连也走专属处理（见
	// claude.CommonClaudeHeadersOperation / claude.FilterBetaFlags）。
	// 通配符 passthrough 会绕过这些处理直接透传客户端原值，导致 Bedrock 返回
	// "Extra inputs are not permitted"、Vertex 返回 "Unexpected value(s) ... for
	// the anthropic-beta header"。这里集中拦截，需要时仍可通过显式 override 配置。
	"anthropic-beta":    {},
	"anthropic-version": {},
...
func DoTaskApiRequest(a TaskAdaptor, c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (*http.Response, error) {
	...
	common2.SysLog(fmt.Sprintf("[TaskSubmit] upstream request URL: %s", fullRequestURL))
	req, err := http.NewRequest(c.Request.Method, fullRequestURL, requestBody)
	if err != nil {
		return nil, fmt.Errorf("new request failed: %w", err)
	}
